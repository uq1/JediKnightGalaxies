...
qboolean G_ClearLineOfSight(const vec3_t point1, const vec3_t point2, int ignore, int clipmask)
{
	...
	trap->Trace( &tr, point1, NULL, NULL, point2, ignore, clipmask , 0, 0, 0);
	if ( tr.fraction == 1.0 ) 
	{
		return qtrue;
	}
	...
	if(EntIsGlass(hit))
	{
		...
		trap->Trace(&tr, newpoint1, NULL, NULL, point2, hit->s.number, clipmask , 0, 0, 0);
...
qboolean CanSee ( gentity_t *ent ) 
{
	...
	trap->Trace( &tr, eyes, NULL, NULL, spot, NPC->s.number, MASK_OPAQUE , 0, 0, 0);
...
qboolean G_ClearLOS( gentity_t *self, const vec3_t start, const vec3_t end )
{
	...
	trap->Trace( &tr, start, NULL, NULL, end, ENTITYNUM_NONE, CONTENTS_OPAQUE/*CONTENTS_SOLID*//*(CONTENTS_SOLID|CONTENTS_MONSTERCLIP)*/ , 0, 0, 0);
	...
	{//can see through 3 panes of glass
		if ( tr.entityNum < ENTITYNUM_WORLD )
		{
			...
			{//can see through glass, trace again, ignoring me
				trap->Trace( &tr, tr.endpos, NULL, NULL, end, tr.entityNum, MASK_OPAQUE , 0, 0, 0);
				traceCount++;
				continue;
			}
		}
