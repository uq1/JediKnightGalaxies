...
void misc_model_breakable_gravity_init( gentity_t *ent, qboolean dropToFloor )
{
	...
	if ( dropToFloor )
	{
		...
		trap->Trace( &tr, top, ent->r.mins, ent->r.maxs, bottom, ent->s.number, MASK_NPCSOLID , 0, 0, 0);
...
void G_PortalifyEntities(gentity_t *ent)
{
	...
	while (i < MAX_GENTITIES)
	{
		...
		if (scan && scan->inuse && scan->s.number != ent->s.number && trap->InPVS(ent->s.origin, scan->r.currentOrigin))
		{
			...
			trap->Trace(&tr, ent->s.origin, vec3_origin, vec3_origin, scan->r.currentOrigin, ent->s.number, CONTENTS_SOLID, 0, 0, 0);
...
void maglock_link( gentity_t *self )
{
	...
	trap->Trace( &trace, start, vec3_origin, vec3_origin, end, self->s.number, MASK_SHOT , 0, 0, 0);
