...
void G_RunExPhys(gentity_t *ent, float gravity, float mass, float bounce, qboolean autoKill, int *g2Bolts, int numG2Bolts)
{
	...
	{ //factor it in before we do anything.
		...
		trap->Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ground, ent->s.number, ent->clipmask, 0, 0, 0);
	...
	{ //nothing to do if we have no velocity even after gravity.
		...
		{ //call touch if we're in something
			trap->Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, ent->r.currentOrigin, ent->s.number, ent->clipmask, 0, 0, 0);
	...
	{ //Have we been passed a bolt index array to clip against points on the skeleton?
		...
		while (i < numG2Bolts)
		{
			...
			trap->Trace(&tr, boneOrg, tMins, tMaxs, projectedBoneOrg, ent->s.number, ent->clipmask, 0, 0, 0);
	...
	trap->Trace(&tr, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, projectedOrigin, ent->s.number, ent->clipmask, 0, 0, 0);
