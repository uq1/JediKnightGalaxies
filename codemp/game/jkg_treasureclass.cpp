...
#ifdef _GAME
extern int JKG_Easy_FindItemByInternal(char *internalName);
extern int lastUsedItemID;
extern itemData_t itemLookupTable[MAX_ITEM_TABLE_SIZE];

#define JKG_FindItem JKG_Easy_FindItemByInternal
#define itemTable itemLookupTable
#define numItems lastUsedItemID
#define itemData itemData_t
#endif
...
TreasureClass::TreasureClass(const char* szName, cJSON* pJSON) {
	...
	if (pJSON == nullptr) {
		// Autogenerated
		TreasureEntry te;
		te.tc.nItem = JKG_FindItem((char*)szName);
		te.nOdds = 1;
		te.bResolved = true;
...
void TreasureClass::PrintDropRates(gentity_t* ent, int nSamples, bool bMulti) {
	...
	for (auto it = mDropCounts.begin(); it != mDropCounts.end(); ++it) {
		...
		itemData* pItemData = &itemTable[nItemID];
...
static void JKG_TC_Autogenerate() {
	for (int i = 0; i < numItems; i++) {
		itemData* pItemData = &itemTable[i];
