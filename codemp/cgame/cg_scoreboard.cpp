...
static void CG_DrawClientScore( int y, score_t *score, float *color, float fade, qboolean largeFormat ) 
{
		...
		(ci->duelTeam == DUELTEAM_LONE || ci->duelTeam == DUELTEAM_DOUBLE))
	{
		...
		{
			CG_DrawPic ( iconx, y, 32, 32, trap->R_RegisterShaderNoMip ( "gfx/mp/pduel_icon_lone" ) );
		}
		else
		{
			CG_DrawPic ( iconx, y, 32, 32, trap->R_RegisterShaderNoMip ( "gfx/mp/pduel_icon_double" ) );
		}
	}
	else
	{
	...
	//--futuza: refactor this stupid, needs its own function
	Text_DrawText(SB_NAME_X, y, ci->name, colorWhite, FONT_MEDIUM, -1, 0.9f*scale);	//new better way, allows us to use RBG colors
